...
export const EmojiCard: React.FunctionComponent<EmojiCardProps> = ({code, emoji, name, category,group, subgroup}) => {
    const [isOpen, setIsOpen] = React.useState(false)
return (
    <div className="bg-[#ffcd7f] p-2 rounded-xl shadow-md flex flex-col items-center">
        <span className="text-4xl md:text-5xl mb-2" role={'img'} aria-label={emoji}>{emoji}</span>
        <h3 className='font-bold text-gray-800 '>{name}</h3>
        <button onClick={()=>setIsOpen(true)} className="text-blue-500 text-sm mt-3">
            more info
        </button>
